...
const init = async () => {
    ...
    await server.register({  //hapi-mongodb 用
   ...
   await server.register(Inert); //Using Inert for sending static files
    ...
    server.route({
        method : 'GET',
        path: '/person',
        handler: async (req,h)=>{   //users콜렉션에서 찾아오기
            const person = await req.mongo.db.collection('users').findOne({"name":"gd"},{projection:{name:0}}); //projection에서 0이면 그것만 안보여주고 1이면 그것만 보여줌
            return person;
        }
    })
    ...
    server.route({
        ...
        handler: async(req,h) =>{
            ...
            data.married = Boolean(data.married);
            data.createdAt = new Date();
            await req.mongo.db.collection('users').insertOne(data);
            return "Done!"
        }
    })
